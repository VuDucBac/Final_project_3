...
class Vehicle_counting:
    def __init__(
        ...
        self.x_rec = 0
        self.y_rec = 0
        self.boundaries = [[([20, 0, 100], [150, 150, 255])],[([0, 160, 160], [150, 255, 255])],[([50, 100, 50], [160, 250, 160])]]
        self.sum = [0,0,0]
    """
            ...
            for i in range(len(self.boundaries)):
                ...
                #print(mask)
                    ...
                    if self.sum.index(np.max(self.sum)) == 0:
                        ...
                        print("yellow")
                    elif self.sum.index(np.max(self.sum)) == 2:
                        print("green")
                print(self.sum)
                print(self.sum.index(np.max(self.sum)))



    def Get_mouse_Rectangle(self, event, x, y, flags, frame) :
        if event == cv2.EVENT_LBUTTONDOWN :
            self.rec.append([(0,0),(0,0)])
            self.rec[self.click_rec][0] = (x,y)
            self.draw = True
            print ("diem dau:{0}".format(self.rec[self.click_rec][0]))
            self.x_rec = x
            self.y_rec = y
        ...
        #        cv2.line(image, (self.x,self.y),(x,y), (255,0,255),2)
            ...
            print ("click couter = {0}".format(self.click_rec))
            self.rec[self.click_rec][1] = (x,y)
            self.click_rec+=1
            self.draw = False

            #print("point1: {0}" .format(self.point1))
            print("rec: {0}" .format(self.rec))
            cv2.rectangle(frame, (self.x_rec,self.y_rec),(x,y), (255,0,255),2)
            cv2.imshow("Set Rec window", frame)
            #self.click_rec+=1
            self.draw = False
            ...
            #cv2.imshow("Set rec2 window", frame)
    ...
    def Set_Rec(self, frame_id, frame):
        if frame_id == 0 :
            ...
            image = cv2.imread("./mot_outputs/First_frame/first_frame.jpg")
            """
            cv2.namedWindow("Set Rec window")
            cv2.setMouseCallback("Set Rec window", self.Get_mouse_Rectangle,frame)
    ...
    def draw_line_vertical(self, frame, point0, point1, i):
        ...
        #        0.7e-3 * frame.shape[0], (0, 255, 255), 2)
    def draw_rec (self, frame, point0, point1):
        cv2.rectangle(frame, point0,point1, (124,252,0),2)
    def draw_line(self, frame):
        ...
        for i in range(len(self.rec)) :
            self.draw_rec(frame,self.rec[i][0],self.rec[i][1])
